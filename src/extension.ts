...
export function activate(ctx: vscode.ExtensionContext) {
    ...
    diagnosticCollection =
        vscode.languages.createDiagnosticCollection("flutter-a11y");
...
class A11yCodeActionProvider implements vscode.CodeActionProvider {
  ...
  child: ${widget}
    ...
    private getWidgetFromDiagnostic(
        doc: vscode.TextDocument,
        range: vscode.Range
    ): string {
...
function analyzeDartFile(document: vscode.TextDocument) {
    ...
    const imageWithoutLabelRegex =
        /Image\.[a-zA-Z]*\((?![\s\S]*semanticLabel)[\s\S]*?\)/g;
    let imageMatch;
    ...
    const buttonRegex =
        /(TextButton|ElevatedButton|OutlinedButton|IconButton)\(\s*(?![\s\S]*onPressed)[\s\S]*?\)/g;
    let buttonMatch;
    ...
    vscode.window.withProgress(
        ...
        async (progress, token) => {
            try {
                ...
                if (total === 0) {
                    vscode.window.showInformationMessage(
                        `No Dart files found in workspace to scan.`
                    );
                    return;
                }
                ...
                for (const fileUri of dartFiles) {
                    if (token.isCancellationRequested) {
                        vscode.window.showInformationMessage(
                            `Scan cancelled after ${processed} files.`
                        );
                        return;
                    }
                    ...
                    try {
                        ...
                        progress.report({
                            ...
                            message: `Scanned: ${processed}/${total} (${path.basename(
                                fileUri.fsPath
                            )})`,
                        });
                    } catch (err) {
                        console.error(
                            `Error processing ${fileUri.fsPath}:`,
                            err
                        );
                    }
                }
                ...
                if (issueCount > 0) {
                    ...
                    const outputChannel = vscode.window.createOutputChannel(
                        "Flutter A11y Scanner"
                    );
                    outputChannel.appendLine(
                        `Flutter Accessibility Scan Results: ${new Date().toLocaleString()}`
                    );
                    outputChannel.appendLine(
                        `Found ${issueCount} accessibility issues in ${filesWithIssues.length} files:`
                    );
                    ...
                    vscode.window.showInformationMessage(
                        `No accessibility issues found in ${total} dart files.`
                    );
                }
            } catch (err) {
                console.error("Error during workspace scan:", err);
                vscode.window.showErrorMessage(
                    `Error scanning workspace: ${err}`
                );
            }
        }
    );
